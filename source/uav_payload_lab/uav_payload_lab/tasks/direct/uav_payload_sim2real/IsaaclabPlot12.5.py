...
DEFAULT_OUT_DIR = os.path.join(SCRIPT_DIR, "PLOT")
...
DEFAULT_SIM_CSV = "/home/shenji/uav_payload_lab/uav_payload_lab/logs/rsl_rl/uav_payload_sim2real_rl/2026-06-21_21-11-18_pa_px4ctbr_measured_phys_goal4_rawreward003_seed42/payload_data.csv"

# === Analysis window (seconds) ===
# CLI 里 --time_window -1 表示不裁剪
DEFAULT_TIME_WINDOW_S = 35.0
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Output directory for generated figures and metrics.",
    )
    parser.add_argument(
        "--time_window",
        type=float,
...
if "UAV_a_wz" in df_sim.columns:
    ...
    fig_path = out_path("plot_4_world_dynamics.png")
...
if "UAV_a_wx" in df_sim.columns and "UAV_a_wy" in df_sim.columns:
    ...
    if len(candidates) == 0:
        ...
        fig_path = out_path("plot_5_input_shaping_evidence_0_5s.png")
        plt.savefig(fig_path, dpi=300)
        print(f"  - {fig_path}")
...
if all([c in df_sim.columns for c in _need_cols]):
    ...
    fig_path = out_path("plot_6_combined.png")
    plt.savefig(fig_path, dpi=300)
...
plt.savefig(out_path("plot_1_payload.png"), dpi=300)

plt.figure(fig2.number)
plt.savefig(out_path("plot_2_uav_states.png"), dpi=300)

plt.figure(fig3.number)
plt.savefig(out_path("plot_3_actions.png"), dpi=300)

print("\n[Output] 所有图像已保存：")
print(f"  - {out_path('plot_1_payload.png')}")
print(f"  - {out_path('plot_2_uav_states.png')}")
print(f"  - {out_path('plot_3_actions.png')}")
